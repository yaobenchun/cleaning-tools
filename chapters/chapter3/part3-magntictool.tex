...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{figure/chapter3/structure-dqc-mag-tool.png}
	\caption{大容量强磁吸附工具总体结构示意图}
	\label{fig:structure_dqc_mag_tool}
\end{figure}

侧吸强磁部件（对应工具上部强磁结构）用于吸附小尺度磁性碎屑，主要由带窗强磁保护管、导磁骨架和永磁块组成，其局部结构见图\ref{fig:structure_magnetic_tool}(b)。永磁块安装于保护管内部，保护管沿周向开窗，使永磁体表面能够与井筒内的磁性碎屑直接接触，增大有效吸附面积；骨架沿轴向加工出吸附沟槽，碎屑主要积存于相邻吸附单元之间的沟槽中，每个吸附单元包含两组永磁块。工具上端设有两个强磁架，两者结构相同，单个强磁架上沿轴向布置5条吸附沟槽，沟槽长度$h=500\,\text{mm}$，沟槽截面面积$s=603\,\text{mm}^2$。

环吸强磁部件（对应工具下部强磁结构）用于吸附大尺度磁性碎屑，采用开放式结构，其局部结构见图\ref{fig:structure_magnetic_tool}(c)。永磁块沿骨架圆周方向均匀布置，其外圆面直接暴露于工具外侧，使大尺寸铁屑能够直接吸附于永磁体表面，避免因保护结构遮挡而降低对粗大碎屑的捕捉能力。工具下端同样设有两个强磁架，两者结构相同，下部强磁的环空面积$s_2=10343\,\text{mm}^2$，环空长度$h_2=1420\,\text{mm}$；相应地，上部强磁的环空面积$s_1=7340\,\text{mm}^2$，环空长度$h_1=1438\,\text{mm}$。

两种强磁部件均设有保护层：上部强磁的永磁块安装于带窗的不锈钢保护管内，下部强磁的永磁块安装于开放式骨架内，保护层可将运输、装配与下井过程中产生的机械冲击以及井筒内的腐蚀性介质与磁材本体隔离开来，磁块在制造时表面还采用镀镍处理。永磁块的固定不采用螺钉连接，各永磁块均可单独拆卸、更换，既便于工具的装配与现场维护，也避免了井下振动引起螺钉松脱的问题。

工具各功能单元之间通过螺纹与芯轴连接，芯轴的连接螺纹为2.85\inch-8 Stub ACME。为防止扭矩与振动作用下螺纹退扣，连接部位设有止退环，并由下接头端面键承担主要防退扣扭矩，下接头端面上设有6个端面键，材料为4145H，键宽$19\,\text{mm}$、键高$11.75\,\text{mm}$，端面键截面为防退扣结构的薄弱环节。各危险截面的螺纹剪切、抗拉、抗扭及防退扣强度校核见\ref{sec:mag_tool_check}节。
...
强磁工具的主要受力件为芯轴，其材料为4330V-155KSI，最小屈服强度为930MPa；强磁骨架选用4145钢，永磁体保护套选用304不锈钢，永磁体选用耐温性与抗退磁能力更好的钐钴（SmCo5）材料。骨架作为工具磁路的一部分，宜选用导磁效果好、可磁化程度高的金属材料，以起到聚集磁感线、增大磁通的作用；永磁体的充磁方式及其对工具表面磁场分布的影响见后文磁铁布置研究。
...
\begin{table}[htbp]
    ...
    \renewcommand{\arraystretch}{1.3} % 增加行高，使表格不拥挤
    \begin{tabularx}{.9\textwidth}{llcX}
        \toprule
        构件 & 结构参数 & 符号 & 数值 \\
        \midrule
        ...
        \midrule
        下接头 & 端面键 & --- & 6个（材料4145H，$19\,\text{mm}\times11.75\,\text{mm}$） \\
        \bottomrule
    \end{tabularx}
\end{table}

注：表中$s$、$s_1$、$s_2$、$h$、$h_1$、$h_2$为强磁工具吸附容量计算所采用的结构参数，其定义见\ref{sec:mag_tool_check}节。

\section[强磁工具结构设计计算与校核]{强磁工具结构设计计算与校核\protect\footnote{合同条款：5.2.1.3井筒高效清洁关键技术配套工具设计——（2）大容量强磁吸附工具：抗拉$>=100T$，抗扭$>=18kN\cdot m$，吸附力$>=8kg/cm^2$，吸附容量$>=50L$}}\label{sec:mag_tool_check}
...
经调研发现，符合井筒清洁工具应用场景的可能材料如表\ref{tab:magnet_params}所示，两种典型磁材：烧结钐钴磁铁（SmCo5）和钕铁硼（N45UH）在不同温度下的退磁曲线如图\ref{fig:demagnetization_SmCo5}、\ref{fig:demagnatization_N45UH}所示。可知，SmCo5在$150\,^\circ\mathrm{C}$-$200\,^\circ\mathrm{C}$的高温环境下，其$H_{cj}$在$-15.8$至$-14.2\mathrm{kOe}$之间；N45UH永磁材料在$150\,^\circ\mathrm{C}$-$200\,^\circ\mathrm{C}$的高温环境下，其$H_{cj}$在$-9.0$至$-6.0\mathrm{kOe}$之间。两数据对比可知，虽然在地面环境中N45UH的表观磁性更强，但是当工具下井后，温度升高，其磁性将大大衰减，于是其使用温度一般不超过$120\,^\circ\mathrm{C}$。SmCo5永磁材料因其$H_{cj}$更高，其耐温性和抗退磁能力更好，考虑到本项目中清洁工具$150\,^\circ\mathrm{C}$的设计耐温指标，于是，选用钐钴材料为宜。
...
该工具可以与模块化多合一套管清洁工具一同使用（推荐），也可以在刮完井之后使用。将强磁吸附工具放在预先放好木板的转盘上（防止强磁吸附工具与转盘发生吸附），并接于钻柱底部，然后提起，取掉木板，准备下井。将大容量强磁吸附工具下到井下离通井位置3-5m处，开泵循环。将大容量强磁吸附工具在该位置上下活动。保持循环，平稳地上提钻柱。起钻时要求平稳、低速、严禁剧烈震动与撞击，以保护强磁部件。

\subsection{强磁工具维护与保养}

大容量强磁吸附工具每次使用后，应放在没有铁屑和杂物区域内的木板或橡胶板上，清除干净吸附在强磁工具表面的金属颗粒和铁屑粉末，然后用清水清洗各零件。工具在井下工作多次后，或在恶劣条件下工作后，应进行维护修理，将其全部拆开、检查、重新装配。

\subsection{强磁工具检验与测试}
...
对于强磁组件，应进行高温烘烤测试。将组装好的磁铁或产品直接放入恒温烤箱中，在有$150^{\circ}\mathrm{C}$下保温2小时。取出冷却至室温后，测量其表磁或磁通量，对比加热前后的衰减比例应小于<5\%。

为保证磁铁对紧贴表面的铁屑（材料以45钢为例）产生 $8 \mathrm{kg/cm²}$ 以上的吸附力\footnote{响应合同内容：5.2.1.3井筒高效清洁关键技术配套工具设计---大容量强磁吸附工具：吸附力$\ge 8 \mathrm{kg/cm²}$}，强磁工具在出厂时应使用磁强计对磁铁表面磁场进行测量，磁铁表面的磁感应强度需要达到 $1.42  (\mathrm{T})$（即 14200 高斯）以上。测量前应清洁磁铁表面，确保磁铁表面平整，无铁屑残留，磁强计探头表面应紧贴磁铁表面，二者之间无间隙。


\section{强磁工具适用井况、管柱组合与施工工艺}
