...
本节选取了两种不同类型的压井液进行比较，分别是牛顿流体的水和氯化钠溶液（比重为1.3）。这两种压井液具有不同的粘度，对铁屑颗粒的携带特性不同。给定$\theta=90^\circ$、$v_{in}=0.27\mathrm{m/s}$条件，通过模拟研究，比较这两种压井液在携带铁屑颗粒方面的效果。通过对不同压井液的性能特点进行分析，可以为选择最适合的压井液粘度提供参考，从而优化洗井操作并提高效率。图1.13描述的是不同压井液粘度下井筒内岩屑颗粒分布情况。从图可以看出，流体粘度对铁屑颗粒运移效率的影响不大。随着压井液粘度的增加，移动铁屑床的速度略微增大。由于随着压井液粘度增大，铁屑颗粒运移效率略微增加，压力出口处的铁屑颗粒有增多现象。
...
\begin{figure}[htbp]
    ...
    \subfloat[$\mu = 1.0\mathrm{mPa}\cdot\mathrm{s}$ 时的铁屑速度分布]{
    ...
    %\vspace{10pt} % 上下两张图之间的垂直间距，可根据需要调整
    % --- 子图 (b) ---
    \subfloat[$\mu = 1.7\mathrm{mPa}\cdot\mathrm{s}$ 时的铁屑速度分布]{
        \begin{minipage}[t]{0.9\linewidth}
            \centering
            % 替换为你的第二张图片文件名
    ...
    % --- 总标题 ---
    \caption{压井液粘度对铁屑颗粒速度分布的影响（$\theta=90^\circ$,$v_{\mathrn{in}=0.27\mathrm{m/s}$）}
